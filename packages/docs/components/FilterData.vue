<template>
  <div>
    <div class="ag-theme-quartz" id="filterDataGrid" style="height: 200px; width: 100%;"></div>
...
onMounted(async () => {
  try {
    const response = await fetch(import.meta.env.BASE_URL + 'results/04_filter.json?' + Date.now())
    ...
    await nextTick()
    const gridDiv = document.querySelector('#filterDataGrid')
    if (gridDiv) {
